...
outputFolder = "\\\\allen/programs/celltypes/workgroups/hct/cellTaxonomy/nomenclature/upload_to_CTTS/human_M1/"
...
```{r}
nomenclature <- read.csv("nomenclature_table_CCN201912131_withExtra.csv")
mappingIn    <- fread("cell_to_cell_set_assignments_CCN201912131.csv")
...
TaxonomyAccessionID <- "CCN202107261"
translate_assignee  <- 
  setNames(c("nik.jorstad@alleninstitute.org","na.com"),
           c("Nikolas Jorstad","na")) # Note, this is just Nik, but I am adding a dummy second to avoid errors
...
Second, let's define the sample.csv.  Note that Uri's for each sample are required, and should either (1) point to a specific location in the Allen Institute LIMS system or (2) be of the following format: urn:example:[Institute acronym]:[Lab PI Surname]:[Unique sample identifier assigned by the lab].  Also note that in the current iteration of the CTTS each sample can only be represented once, so we are only including mapping to cell types (e.g., leaf nodes).  In this case, we are linking to LIMS ids.
  
Read in meta-data for LIMS for the URI.  
  
```{r}
# Read in metadata
metadata   <- read.csv("cell_metadata_CCN201912131.csv")

# Variable preparation
dataset    <- "M1_human_miniatlas"
ExternalId <- metadata$barcode
uri        <- paste0("http://lims2.corp.alleninstitute.org/rseq_library_preps/",metadata$lims_id,
                     ".json?barcode=",ExternalId)
```
  
  
Now make and save the sample files.  
...
```{r}
create_zip_archive("human_m1_miniatlas_v2")  # Optional zip and manifest file prefix
```
