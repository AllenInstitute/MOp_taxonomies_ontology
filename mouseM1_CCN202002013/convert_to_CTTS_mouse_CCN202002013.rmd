...
TaxonomyAccessionID <- "CCN202002013"
translate_assignee  <- 
  setNames(c("zizheny@alleninstitute.org","nik.jorstad@alleninstitute.org","scala@bcm.edu","bosiljkat@alleninstitute.org"),
           c("Zizhen Yao","Nikolas Jorstad","Federico Scala","Bosiljka Tasic"))
...
Second, let's define the sample.csv.  Note that Uri's for each sample are required, and should either (1) point to a specific location in the Allen Institute LIMS system or (2) be of the following format: urn:example:[Institute acronym]:[Lab PI Surname]:[Unique sample identifier assigned by the lab].  Also note that in the current iteration of the CTTS each sample can only be represented once, so we are only including mapping to cell types (e.g., leaf nodes).  In this case, the mouse data are from Evan Macoscko's lab, so we format the URIs as below.  
  
```{r}
# Variable preparation
dataset    <- "M1_mouse_miniatlas"
uri        <- paste0("urn:example:Broad:Macosko:",samples)
...
# Output a version using cell set accession IDs for the ontology.
cellSetHierarchy <- find_cell_set_family(nomenclature$cell_set_accession, nomenclature)
colnames(cellSetHierarchy) <- c("Parent_accession","Child_accession")
fwrite(as.data.frame(cellSetHierarchy),paste0("CellSetHierchy_",TaxonomyAccessionID,".csv"))
```
...
```{r}
create_zip_archive("mouse_m1_miniatlas_v6")  # Optional zip and manifest file prefix
```
