...
outputFolder = "\\\\allen/programs/celltypes/workgroups/hct/cellTaxonomy/nomenclature/cell_type_cards_taxonomies/mouse_M1/"
...
updated_nomenclature <- read.csv("nomenclature_table_CCN202002013.csv")
```
...
write.csv(updated_nomenclature,"nomenclature_table_CCN202002013.csv",row.names=FALSE)
```
...
```{r}
load("dendrogram_CCN202002013.RData")
dend <- as.dendrogram(updated_dendrogram)

# Need to overwrite the existing information to get this working
taxonomy_id <- updated_nomenclature$taxonomy_id[1]
nomenclature_information <- build_nomenclature_table(dend,taxonomy_id = taxonomy_id)
```
...
```{r}
# First build the original_label
updated_nomenclature$original_label <- updated_nomenclature$cell_set_preferred_alias
rownames(updated_nomenclature) <- updated_nomenclature$cell_set_accession
updated_nomenclature[nomenclature_information$cell_set_information$cell_set_accession,"original_label"] <- nomenclature_information$cell_set_information$original_label

# Now update the dendrogram
updated_dendrogram <- update_dendrogram_with_nomenclature(nomenclature_information$initial_dendrogram,updated_nomenclature)

# Now output the plot as a sanity check
...
save(updated_dendrogram, file=paste0("updated_dendrogram_",taxonomy_id,".RData"))
```
...
out <- file(paste0("updated_dendrogram_",taxonomy_id,".json"), open = "w")
...
metadata  <- read.csv("perCell_mouse_annotation_12082020.csv")
...
label_col <- "cluster_update"  # Column name with dendrogram labels
metadata[,label_col] <- gsub("L5 PT","L5 ET",metadata[,label_col])
...
```{r} 
fwrite(mapping,"cell_to_cell_set_assignments_CCN202002013.csv")
```    
