...
```{r}
nomenclature <- read.csv("nomenclature_table_CCN201912132.csv")
mappingIn    <- fread("cell_to_cell_set_assignments_CCN201912132.csv")
...
TaxonomyAccessionID <- "CCN202107262"
translate_assignee  <- 
  setNames(c("nik.jorstad@alleninstitute.org","fennak@gmail.com"),
           c("Nikolas Jorstad","Fenna Krienen"))
...
Second, let's define the sample.csv.  Note that Uri's for each sample are required, and should either (1) point to a specific location in the Allen Institute LIMS system or (2) be of the following format: urn:example:[Institute acronym]:[Lab PI Surname]:[Unique sample identifier assigned by the lab].  Also note that in the current iteration of the CTTS each sample can only be represented once, so we are only including mapping to cell types (e.g., leaf nodes).  In this case, the marmoset data are from the Feng lab, so we format the URIs as below.  
  
```{r}
# Variable preparation
dataset    <- "M1_marmoset_miniatlas"
uri        <- paste0("urn:example:MIT:Feng:",samples)
...
```{r}
create_zip_archive("marmoset_m1_miniatlas_v2")  # Optional zip and manifest file prefix
```
